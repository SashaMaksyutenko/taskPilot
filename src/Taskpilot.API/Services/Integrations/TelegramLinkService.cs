...
/// <summary>
/// Handles Telegram account linking. One-time codes are kept in the distributed
/// cache (Redis or in-memory) for 10 minutes, keyed by code.
/// </summary>
public class TelegramLinkService : ITelegramLinkService
{
    private static readonly TimeSpan CodeTtl = TimeSpan.FromMinutes(10);
    private const string CachePrefix = "tg-link:";
    ...
    /// <inheritdoc />
    public async Task<Result<(string Code, string BotUsername)>> CreateLinkCodeAsync(Guid userId)
    {
        if (!_options.IsConfigured)
            return Result<(string, string)>.Fail("Telegram bot is not configured.");
        ...
        return Result<(string, string)>.Ok((code, _options.BotUsername));
    }

    /// <inheritdoc />
    public async Task<bool> LinkByCodeAsync(string code, string chatId)
    {
        ...
        _logger.LogInformation("Telegram linked. UserId: {UserId}", userId);
        return true;
    }
    ...
    /// <inheritdoc />
    public async Task<Result<TelegramLinkStatus>> GetStatusAsync(Guid userId)
    {
        ...
        return Result<TelegramLinkStatus>.Ok(new TelegramLinkStatus(!string.IsNullOrEmpty(chatId), _options.BotUsername));
    }
}
