...
function AuthImage({ src, alt }: { src?: string; alt?: string }) {
  ...
  if (!url) return null
  return <img src={url} alt={alt ?? ''} className="my-2 max-h-80 rounded-lg border border-border" />
}
    ...
    <div className="text-sm leading-relaxed text-foreground">
      <ReactMarkdown
        ...
        components={{
          ...
          blockquote: ({ children }) => (
            <blockquote className="mb-2 border-l-2 border-border pl-3 text-muted">
              {children}
            </blockquote>
          ),
          code: ({ children }) => (
            <code className="rounded bg-canvas px-1 py-0.5 font-mono text-[0.85em]">{children}</code>
          ),
          pre: ({ children }) => (
            <pre className="mb-2 overflow-x-auto rounded-lg bg-canvas p-3 font-mono text-xs">{children}</pre>
          ),
        }}
      >
        {children}
