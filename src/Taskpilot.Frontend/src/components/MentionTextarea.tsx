...
type Props = Omit<TextareaHTMLAttributes<HTMLTextAreaElement>, 'value' | 'onChange' | 'onKeyDown'> & {
  value: string
...
/**
 * A textarea with @mention autocomplete. As the user types "@…", a dropdown of
 * matching candidates appears; selecting one inserts "@Name" (name without spaces,
 * matching how the backend resolves mentions).
 */
export default function MentionTextarea({ value, onChange, candidates, onKeyDown, ...rest }: Props) {
        ...
        <ul className="absolute bottom-full left-0 z-30 mb-1 max-h-56 w-full overflow-y-auto rounded-lg border border-slate-300 bg-white shadow-lg dark:border-slate-600 dark:bg-slate-800">
          {matches.map((c, i) => (
            <li key={c.id}>
              <button
                type="button"
                // onMouseDown so the click lands before the textarea blur.
                onMouseDown={(e) => {
                  e.preventDefault()
                  insert(c)
                }}
